  ...
  public function build() {
    ...
    if(isset($ins_media['data'])){
      foreach ($ins_media['data'] as $vm): 
          ...
          if($count == $i){ break;}
          $i++;
          $caption = $vm['caption']['text'];
          $img = $vm['images']['thumbnail']['url'];
          $img_2 = $vm['images']['standard_resolution']['url'];
          $link = $vm["link"];
          $out .= '<div class="sca_bordered_block sca_image_bck sca_bordered_zoom"><a href="'.$link.'" target="_blank"></a><div class="sca_image_over sca_image_bck" data-image="'.$img_2.'"></div><div class="sca_box_content text-center"><div class="sca_bottom_title sca_hidden_title"><p>'.$caption.'</p></div></div></div>';
      endforeach;
    }
    //kint($ins_media);
    
    return [
      '#markup' => '<div id="scalac-instagram-carousel">' . $out . '</div>',
    ];
  }
